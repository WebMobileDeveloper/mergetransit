...
<script>
   ...
   $(function(){
        $(".back_button").click(function(){
            location.href = "{{URL::to('sadmin/details')}}"
        });
        ...
        $("#calcu_distance").click(function(e){
         ...
         $(".add_company_button").click(function(e){
            e.preventDefault();
            location.href = "{{URL::to('/sadmin/contactlist/create')}}"
        })
        ...
        if ($(obj).attr("itemid") != '0') {
            ...
            $.ajaxSetup({
                ...
                $.ajax({
                    type: "POST",
                    dataType:'json',
                    url: "{{ url('sadmin/delete_detail_address') }}",
                    data: {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if(data.status=='success'){
                            ...
                            if(zip_type == 1) {
                                $self.parent().parent().find('.origin_city').val(address.city);
                                $self.parent().parent().find('.origin_province').val(address.state);
                            } else if(zip_type == 2){
                                $self.parent().parent().find('.destination_city').val(address.city);
                                $self.parent().parent().find('.destination_province').val(address.state);
                            }
                        
                        } else {
                            $self.removeClass('has-error').addClass('has-error')
                            if(zip_type === 1) {
                                $self.parent().parent().find('.origin_city').val('');
                                $self.parent().parent().find('.origin_province').val('');
                            } else if(zip_type == 2){
                                $self.parent().parent().find('.destination_city').val('');
                                $self.parent().parent().find('.destination_province').val('');
                            }
                            return false;
                        }
    ...
    function delete_file(id,no){
    	if(confirm("are you sure?")){
    		var url = "{{ url('sadmin/delete_file') }}";
        	url = url + "/" + id + "/" + no;
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box box-info">
                    ...
                    <form class="form-horizontal" id="user_form" name="user_form" action="{{url('sadmin/details/'.$detail->id )}}" method = "post" enctype="multipart/form-data">
