      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            ...
            <small class="pull-right">Date: {{$invoice->due_date}}</small>
          ...
          <table class="table table-striped">
            ...
            <tbody class="invoice_detail_tbody">
                ...
                @foreach($invoice_specials as $special)
                  <tr class="sp_tr" style=";background:#ecefe9">
                      <td >
                          <textarea rows="1" class="form-control" name="activity[]" required >{{$special->activity}}</textarea>  
                      </td>
                      <td class="sp_qty"><input class="form-control" onkeyup="calc($(this))" name="sp_qty[]" placeholder="QTY" type="text" value="{{$special->qty}}" required></td>
                      <td class="sp_rate"><input class="form-control" onkeyup="calc($(this))" name="sp_rate[]" placeholder="rate" type="text" value="{{$special->rate}}" required></td>
                      <td class="sp_amount"><input class="form-control" id="to_amount" name="sp_amount[]"  placeholder="" value="{{$special->amount}}" type="text"></td>
                      <td></td>
                  </tr>
                  <?php
                  $total_amount+=$special->amount;
                  $n++;
                  ?>
                @endforeach
                @endif
                <input name="special_count" value="{{count($invoice_specials)}}" type="hidden" />
            </tbody>
          </table>
        </div>
        <!-- /.col -->
      ...
      <div class="row no-print">
        <div class="col-xs-12">
          ...
          {{--  <div class="box-header with-border">  --}}
            <a onclick="generate_invoice()" class="btn btn-info ">Resend Invoice</a>
          {{--  </div>  --}}
          {{--  @endif  --}}
