
@extends("layout.layout")
@section("contents")
 <!-- HERO -->
 <section class="hero">
    <div class="background"></div>
</section>	
<!-- /HERO -->

<!-- Services -->
<section id="services" class="services">
    <div class="container">
        <div class="row">
            <h2 hidden>
                Services
            </h2>
            <div class="col-md-6">
                <div class="services__item">
                    <h3 class="f--blue">
                    ORGANIZATION PROGRAM
                    </h3>
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="470.586px" height="470.586px" viewBox="0 0 470.586 470.586" style="enable-background:new 0 0 470.586 470.586;" xml:space="preserve">
                            <g>
                                <path d="M327.081,0H90.234C74.331,0,61.381,12.959,61.381,28.859v412.863c0,15.924,12.95,28.863,28.853,28.863H380.35
                                    c15.917,0,28.855-12.939,28.855-28.863V89.234L327.081,0z M333.891,43.184l35.996,39.121h-35.996V43.184z M384.972,441.723
                                    c0,2.542-2.081,4.629-4.635,4.629H90.234c-2.55,0-4.619-2.087-4.619-4.629V28.859c0-2.548,2.069-4.613,4.619-4.613h219.411v70.181
                                    c0,6.682,5.443,12.099,12.129,12.099h63.198V441.723z M128.364,128.89H334.15c5.013,0,9.079,4.066,9.079,9.079
                                    c0,5.013-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079C119.285,132.957,123.352,128.89,128.364,128.89z
                                     M343.229,198.98c0,5.012-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15
                                    C339.163,189.901,343.229,193.968,343.229,198.98z M343.229,257.993c0,5.013-4.066,9.079-9.079,9.079H128.364
                                    c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15C339.163,248.914,343.229,252.98,343.229,257.993z
                                     M343.229,318.011c0,5.013-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15
                                    C339.163,308.932,343.229,312.998,343.229,318.011z"></path>
                            </g>
                        </svg>
                    <ul>
                        <li>
                            Personal admin + Mobile app
                        </li>
                        <li>
                            Admin enters all orders and details into System
                        </li>
                        <li>
                            Custom business reports 
                        </li>
                        <li>
                            Organize shipment documents via Mobile App
                        </li>
                        <li>
                            Create invoices with one touch
                        </li>
                        <li>
                            Email all shipments docs direct from App.
                        </li>
                    </ul>
                    <strong class="f--blue">
                        $99  / MONTH
                    </strong>
                    <div class="btn-box">
                        <button class="f--white" data-class="show--organization">
                            SHOW MORE
                        </button>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="services__item">
                    <h3 class="f--blue">
                        OPTIMIZATION PROGRAM
                    </h3>
                    <svg 
                        version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 212.755 212.755" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 212.755 212.755">
                      <g>
                        <path d="M106.377,0C47.721,0,0,47.721,0,106.377s47.721,106.377,106.377,106.377s106.377-47.721,106.377-106.377   S165.034,0,106.377,0z M106.377,198.755C55.44,198.755,14,157.314,14,106.377S55.44,14,106.377,14s92.377,41.44,92.377,92.377   S157.314,198.755,106.377,198.755z"/>
                        <path d="m113.377,100.096v-39.744c3.961,1.471 7.417,4.17 9.82,7.82 2.127,3.229 6.468,4.123 9.696,1.997 3.229-2.126 4.123-6.467 1.996-9.696-5.029-7.636-12.778-12.82-21.512-14.647v-11.12c0-3.866-3.134-7-7-7s-7,3.134-7,7v11.099c-15.493,3.23-27.168,16.989-27.168,33.426 0,16.437 11.676,30.198 27.168,33.428v39.744c-3.961-1.471-7.417-4.17-9.82-7.82-2.127-3.229-6.468-4.124-9.696-1.997-3.229,2.126-4.123,6.467-1.996,9.696 5.029,7.636 12.778,12.82 21.512,14.647v11.119c0,3.866 3.134,7 7,7s7-3.134 7-7v-11.098c15.493-3.23 27.168-16.989 27.168-33.426-2.84217e-14-16.437-11.675-30.198-27.168-33.428zm-27.168-20.865c0-8.653 5.494-16.027 13.168-18.874v37.748c-7.674-2.847-13.168-10.221-13.168-18.874zm27.168,73.166v-37.748c7.674,2.847 13.168,10.221 13.168,18.874s-5.493,16.027-13.168,18.874z"/>
                      </g>
                    </svg>
                    <ul>
                        <li>
                            Organization program +
                        </li>
                        <li>
                            Dedicated carrier agent 
                        </li>
                        <li>
                            Routing service  
                        </li>
                        <li>
                            All loads booked
                        </li>
                        <li>
                            All communication with brokers
                        </li>
                        <li>
                            Improved All Mile RPM
                        </li>
                    </ul>
                    
                    <strong class="f--blue">
                        $125  / WEEKLY
                    </strong>
                    <div class="btn-box">
                        <button class="f--white" data-class="show--optymalization">
                            SHOW MORE
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- / Services -->

<!-- Features -->
<section class="features">
    <div class="container-fluid">
        <div class="row">
            <div class="col-xl-12 wrapper ">
                <!-- organization -->
                <div class="organization">
                    <h2 class="f--blue">
                        ORGANIZATION PROGRAM
                    </h2>
                    <!-- phone -->
                    <img  class="phone" src="{{asset('assets/images/phone-white.png')}}" alt="" >
                    <!-- phone screen -->
                    <div class="phone__screen screen--1">
                        <!-- screen 1 -->
                        <div class="phone__screen-b phone__screen--1">
                            <span class="number f--blue">
                                1
                            </span>
                            <h3 class="f--blue">
                                Acces to 
                                <br>
                                agent admin
                            </h3>
                            <p>
                                Access to Agent Admin to enter all shipments and details into your reporting system, which is accessed via Mobile App or Web Platform
                            </p>
                        </div>
                        <!-- screen 2 -->
                        <div class="phone__screen-b phone__screen--2">
                            <span class="number f--blue">
                                2
                            </span>
                            <h3 class="f--blue">
                                Individual 
                                <br>
                                Shipment Reports
                            </h3>
                            <p>
                                Individual Shipment Reports with ability to upload pictures or documents and attach to each shipment, from phone or computer images or files.
                            </p>
                        </div>
                        <!-- screen 3 -->
                        <div class="phone__screen-b phone__screen--3">
                            <span class="number f--blue">
                                3
                            </span>
                            <h3 class="f--blue">
                                Pull
                                <br>
                                Reports
                            </h3>
                            <p>
                                Pull reports by date range to show total number of shipments, loaded and deadhead miles, revenue generated, loaded RPM, Total Mile RPM and much more.
                            </p>
                        </div>
                        <!-- screen 4 -->
                        <div class="phone__screen-b phone__screen--4">
                            <span class="number f--blue">
                                4
                            </span>
                            <h3 class="f--blue">
                                Calendar
                            </h3>
                            <p>
                                Calendar View to see what loads you have for the day or drivers to see what loads they have assigned to them.
                            </p>
                        </div>
                        <!-- screen 5 -->
                        <div class="phone__screen-b phone__screen--5">
                            <span class="number f--blue">
                                5
                            </span>
                            <h3 class="f--blue">
                                ADD NEW
                                <br>
                                DRIVERS
                            </h3>
                            <p>
                                Ability to add new drivers as your fleet grows and so much more
                            </p>
                        </div>
                        <!-- screen 6 -->
                        <div class="phone__screen-b phone__screen--6">
                            <span class="number f--blue">
                                6
                            </span>
                            <h3 class="f--blue">
                                Generate Invoices
                            </h3>
                            <p>
                                Generate Invoices with one button upon completion of delivery and send with all paperwork via the App.
                            </p>
                        </div>
                    </div>
                    
                    <!-- icons -->
                    <button class="icon icon--1 active" data-number="1">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 		xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 474.565 474.565" style="enable-background:new 0 0 474.565 474.565;" xml:space="preserve">
                            <g>
                                <path d="M255.204,102.3c-0.606-11.321-12.176-9.395-23.465-9.395C240.078,95.126,247.967,98.216,255.204,102.3z"/>
                                <path d="M134.524,73.928c-43.825,0-63.997,55.471-28.963,83.37c11.943-31.89,35.718-54.788,66.886-63.826
                                    C163.921,81.685,150.146,73.928,134.524,73.928z"/>
                                <path d="M43.987,148.617c1.786,5.731,4.1,11.229,6.849,16.438L36.44,179.459c-3.866,3.866-3.866,10.141,0,14.015l25.375,25.383
                                    c1.848,1.848,4.38,2.888,7.019,2.888c2.61,0,5.125-1.04,7.005-2.888l14.38-14.404c2.158,1.142,4.55,1.842,6.785,2.827
                                    c0-0.164-0.016-0.334-0.016-0.498c0-11.771,1.352-22.875,3.759-33.302c-17.362-11.174-28.947-30.57-28.947-52.715
                                    c0-34.592,28.139-62.739,62.723-62.739c23.418,0,43.637,13.037,54.43,32.084c11.523-1.429,22.347-1.429,35.376,1.033
                                    c-1.676-5.07-3.648-10.032-6.118-14.683l14.396-14.411c1.878-1.856,2.918-4.38,2.918-7.004c0-2.625-1.04-5.148-2.918-7.004
                                    l-25.361-25.367c-1.94-1.941-4.472-2.904-7.003-2.904c-2.532,0-5.063,0.963-6.989,2.904l-14.442,14.411
                                    c-5.217-2.764-10.699-5.078-16.444-6.825V9.9c0-5.466-4.411-9.9-9.893-9.9h-35.888c-5.451,0-9.909,4.434-9.909,9.9v20.359
                                    c-5.73,1.747-11.213,4.061-16.446,6.825L75.839,22.689c-1.942-1.941-4.473-2.904-7.005-2.904c-2.531,0-5.077,0.963-7.003,2.896
                                    L36.44,48.048c-1.848,1.864-2.888,4.379-2.888,7.012c0,2.632,1.04,5.148,2.888,7.004l14.396,14.403
                                    c-2.75,5.218-5.063,10.708-6.817,16.438H23.675c-5.482,0-9.909,4.441-9.909,9.915v35.889c0,5.458,4.427,9.908,9.909,9.908H43.987z"
                                    />
                                <path d="M354.871,340.654c15.872-8.705,26.773-25.367,26.773-44.703c0-28.217-22.967-51.168-51.184-51.168
                                    c-9.923,0-19.118,2.966-26.975,7.873c-4.705,18.728-12.113,36.642-21.803,52.202C309.152,310.022,334.357,322.531,354.871,340.654z
                                    "/>
                                <path d="M460.782,276.588c0-5.909-4.799-10.693-10.685-10.693H428.14c-1.896-6.189-4.411-12.121-7.393-17.75l15.544-15.544
                                    c2.02-2.004,3.137-4.721,3.137-7.555c0-2.835-1.118-5.553-3.137-7.563l-27.363-27.371c-2.08-2.09-4.829-3.138-7.561-3.138
                                    c-2.734,0-5.467,1.048-7.547,3.138l-15.576,15.552c-5.623-2.982-11.539-5.481-17.751-7.369v-21.958
                                    c0-5.901-4.768-10.685-10.669-10.685H311.11c-2.594,0-4.877,1.04-6.739,2.578c3.26,11.895,5.046,24.793,5.046,38.552
                                    c0,8.735-0.682,17.604-1.956,26.423c7.205-2.656,14.876-4.324,22.999-4.324c36.99,0,67.086,30.089,67.086,67.07
                                    c0,23.637-12.345,44.353-30.872,56.303c13.48,14.784,24.195,32.324,31.168,51.976c1.148,0.396,2.344,0.684,3.54,0.684
                                    c2.733,0,5.467-1.04,7.563-3.13l27.379-27.371c2.004-2.004,3.106-4.721,3.106-7.555s-1.102-5.551-3.106-7.563l-15.576-15.552
                                    c2.982-5.621,5.497-11.555,7.393-17.75h21.957c2.826,0,5.575-1.118,7.563-3.138c2.004-1.996,3.138-4.72,3.138-7.555
                                    L460.782,276.588z"/>
                                <path d="M376.038,413.906c-16.602-48.848-60.471-82.445-111.113-87.018c-16.958,17.958-37.954,29.351-61.731,29.351
                                    c-23.759,0-44.771-11.392-61.713-29.351c-50.672,4.573-94.543,38.17-111.145,87.026l-9.177,27.013
                                    c-2.625,7.773-1.368,16.338,3.416,23.007c4.783,6.671,12.486,10.631,20.685,10.631h315.853c8.215,0,15.918-3.96,20.702-10.631
                                    c4.767-6.669,6.041-15.234,3.4-23.007L376.038,413.906z"/>
                                <path d="M120.842,206.782c0,60.589,36.883,125.603,82.352,125.603c45.487,0,82.368-65.014,82.368-125.603
                                    C285.563,81.188,120.842,80.939,120.842,206.782z"/>
                            </g>
                        </svg>
                    </button>
                    <button class="icon icon--2" data-number="2">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 
                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                            <g>
                                <g>
                                    <path d="M389.513,87.422c0-12.012-4.688-23.32-13.184-31.816l-42.422-42.422C325.529,4.805,313.636,0,301.8,0h-2.578v90h90.292
                                        L389.513,87.422L389.513,87.422z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M273.937,309.537c2.871-8.716,7.881-16.831,14.414-23.408l101.562-101.153V120h-105.4c-8.291,0-14.513-6.709-14.513-15V0
                                        H45C20.186,0,0,20.186,0,45v422c0,24.814,20.186,45,45,45h299.513c24.814,0,45.4-20.186,45.4-45V355.049l-16.484,16.084
                                        c-6.679,6.621-14.501,11.44-23.32,14.385l-47.695,15.923l-7.266,0.396c-12.012,0-23.379-5.845-30.439-15.63
                                        c-7.002-9.741-8.906-22.368-5.098-33.779L273.937,309.537z M75,270h149.513c8.291,0,15,6.709,15,15c0,8.291-6.709,15-15,15H75
                                        c-8.291,0-15-6.709-15-15C60,276.709,66.709,270,75,270z M60,225c0-8.291,6.709-15,15-15h149.513c8.291,0,15,6.709,15,15
                                        s-6.709,15-15,15H75C66.709,240,60,233.291,60,225z M60,345c0-8.291,6.709-15,15-15h149.513c8.291,0,15,6.709,15,15
                                        c0,8.291-6.709,15-15,15H75C66.709,360,60,353.291,60,345z M284.513,420c8.291,0,15,6.709,15,15c0,8.291-6.708,15-15,15h-90
                                        c-8.291,0-15-6.709-15-15c0-8.291,6.709-15,15-15H284.513z M75,180c-8.291,0-15-6.709-15-15s6.709-15,15-15h209.513
                                        c8.291,0,15,6.709,15,15s-6.709,15-15,15H75z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M301.111,322.808l-13.05,39.151c-1.956,5.865,3.625,11.444,9.49,9.485l39.128-13.068L301.111,322.808z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M417.609,199.307l-98.789,98.789l42.605,42.605c22.328-22.332,65.773-65.783,98.784-98.794L417.609,199.307z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M503.185,156.284c-5.273-5.303-13.037-8.335-21.27-8.335c-8.233,0-15.996,3.032-21.299,8.35l-21.797,21.797l42.598,42.598
                                        c11.933-11.934,20.181-20.182,21.799-21.799C514.933,187.16,514.932,168.046,503.185,156.284z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <g>
                                        <path d="M503.215,198.896c0.001,0,0.001-0.001,0.002-0.002c0.038-0.038,0.055-0.055,0.086-0.086
                                            C503.272,198.84,503.255,198.857,503.215,198.896z"/>
                                        <path d="M503.303,198.808c0.048-0.048,0.104-0.104,0.133-0.133C503.406,198.705,503.351,198.76,503.303,198.808z"/>
                                        <path d="M503.436,198.675C503.533,198.578,503.535,198.576,503.436,198.675L503.436,198.675z"/>
                                    </g>
                                </g>
                            </g>
                        </svg>
                    </button>
                    <button class="icon icon--3" data-number="3">
                        <svg 
                                version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 			xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                 viewBox="0 0 334.5 334.5" style="enable-background:new 0 0 334.5 334.5;" xml:space="preserve">
                            <path d="M332.797,13.699c-1.489-1.306-3.608-1.609-5.404-0.776L2.893,163.695c-1.747,0.812-2.872,2.555-2.893,4.481
                                s1.067,3.693,2.797,4.542l91.833,45.068c1.684,0.827,3.692,0.64,5.196-0.484l89.287-66.734l-70.094,72.1
                                c-1,1.029-1.51,2.438-1.4,3.868l6.979,90.889c0.155,2.014,1.505,3.736,3.424,4.367c0.513,0.168,1.04,0.25,1.561,0.25
                                c1.429,0,2.819-0.613,3.786-1.733l48.742-56.482l60.255,28.79c1.308,0.625,2.822,0.651,4.151,0.073
                                c1.329-0.579,2.341-1.705,2.775-3.087L334.27,18.956C334.864,17.066,334.285,15.005,332.797,13.699z"/>
                            <g>
                        </svg>
                    </button>
                    <button class="icon icon--4" data-number="4">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                <g>
                                    <g>
                                        <path d="M452,40h-24V0h-40v40H124V0H84v40H60C26.916,40,0,66.916,0,100v352c0,33.084,26.916,60,60,60h392
                                            c33.084,0,60-26.916,60-60V100C512,66.916,485.084,40,452,40z M472,452c0,11.028-8.972,20-20,20H60c-11.028,0-20-8.972-20-20V188
                                            h432V452z M472,148H40v-48c0-11.028,8.972-20,20-20h24v40h40V80h264v40h40V80h24c11.028,0,20,8.972,20,20V148z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="76" y="230" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="156" y="230" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="236" y="230" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="316" y="230" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="396" y="230" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="76" y="310" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="156" y="310" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="236" y="310" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="316" y="310" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="76" y="390" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="156" y="390" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="236" y="390" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="316" y="390" width="40" height="40"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <rect x="396" y="310" width="40" height="40"/>
                                    </g>
                                </g>
                        </svg>
                    </button>
                    <button class="icon icon--5" data-number="5">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 
                            xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                                 width="612px" height="612px" viewBox="0 0 612 612" style="enable-background:new 0 0 612 612;" xml:space="preserve">
                                <g>
                                    <g id="group-add">
                                        <path d="M204,267.75h-76.5v-76.5h-51v76.5H0v51h76.5v76.5h51v-76.5H204V267.75z M459,293.25c43.35,0,76.5-33.15,76.5-76.5
                                            s-33.15-76.5-76.5-76.5c-7.65,0-15.3,2.55-22.95,2.55c15.3,22.95,22.95,45.9,22.95,73.95s-7.65,51-22.95,73.95
                                            C443.7,290.7,451.35,293.25,459,293.25z M331.5,293.25c43.35,0,76.5-33.15,76.5-76.5s-33.15-76.5-76.5-76.5
                                            c-43.35,0-76.5,33.15-76.5,76.5S288.15,293.25,331.5,293.25z M499.8,349.35c20.4,17.851,35.7,43.351,35.7,71.4v51H612v-51
                                            C612,382.5,550.8,357,499.8,349.35z M331.5,344.25c-51,0-153,25.5-153,76.5v51h306v-51C484.5,369.75,382.5,344.25,331.5,344.25z"
                                            />
                                    </g>
                                </g>
                        </svg>
                    </button>
                    <button class="icon icon--6" data-number="6">
                        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 
                                xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 21.001 21.001" xml:space="preserve">
                                <g>
                                    <g>
                                        <path style="" d="M3.001,15.501c-0.552,0-1-0.448-1-1V8.119L1.448,8.395C0.953,8.643,0.353,8.441,0.106,7.948
                                            C-0.141,7.454,0.06,6.854,0.554,6.606l2-1c0.31-0.155,0.678-0.139,0.973,0.044c0.294,0.183,0.474,0.504,0.474,0.851v8
                                            C4.001,15.053,3.553,15.501,3.001,15.501z"/>
                                    </g>
                                    <g>
                                        <path style="" d="M12.001,15.501h-5c-0.404,0-0.769-0.244-0.924-0.617c-0.155-0.374-0.069-0.804,0.217-1.09l4-4
                                            c0.254-0.254,0.394-0.591,0.394-0.95c0-0.358-0.14-0.695-0.394-0.949c-0.508-0.508-1.39-0.508-1.9,0.001
                                            C8.141,8.148,8.001,8.485,8.001,8.844c0,0.552-0.448,1-1,1s-1-0.448-1-1c0-0.894,0.348-1.733,0.98-2.364
                                            c1.265-1.263,3.464-1.263,4.727,0.001c0.632,0.631,0.979,1.471,0.979,2.363c0,0.893-0.348,1.733-0.979,2.364l-2.293,2.293h2.586
                                            c0.552,0,1,0.448,1,1S12.553,15.501,12.001,15.501z"/>
                                    </g>
                                    <path style="" d="M19.956,9.878c0.338-0.457,0.545-1.016,0.545-1.627c0-1.517-1.234-2.75-2.75-2.75
                                        c-1.031,0-1.966,0.569-2.44,1.484c-0.254,0.49-0.063,1.094,0.428,1.348c0.49,0.254,1.094,0.062,1.348-0.428
                                        c0.128-0.249,0.383-0.404,0.664-0.404c0.414,0,0.75,0.336,0.75,0.75s-0.336,0.75-0.75,0.75c-0.552,0-1,0.448-1,1s0.448,1,1,1
                                        c0.689,0,1.25,0.561,1.25,1.25s-0.561,1.25-1.25,1.25s-1.25-0.561-1.25-1.25c0-0.552-0.448-1-1-1c-0.552,0-1,0.448-1,1
                                        c0,1.792,1.458,3.25,3.25,3.25s3.25-1.458,3.25-3.25C21.001,11.312,20.595,10.472,19.956,9.878z"/>
                                </g>
                        </svg>
                    </button>
                </div>

                <!-- organization -->
                <div class="optymalization">
                    <h2 class="f--blue">
                        OPTIMIZATION PROGRAM
                    </h2>
                    <!-- phone -->
                    <img  class="phone" src="{{asset('assets/images/phone-white.png')}}" alt="" >
                    <!-- phone screen -->
                    <div class="phone__screen screen--1">
                        <!-- screen 1 -->
                        <div class="phone__screen-b phone__screen--1">
                            <span class="number f--blue">
                                1
                            </span>
                            <h3 class="f--blue">
                                All Organization
                                <br>
                                Program Services+
                            </h3>
                            <p>
                                Receive all the benefits of the Organization Program
                            </p>
                        </div>
                        <!-- screen 2 -->
                        <div class="phone__screen-b phone__screen--2">
                            <span class="number f--blue">
                                2
                            </span>
                            <h3 class="f--blue">
                                Dedicated 
                                <br>
                                Carrier Agent 
                            </h3>
                            <p>
                                You will be assigned a Dedicated Carrier Agent who will work with you and get to learn your business needs fully.
                            </p>
                        </div>
                        <!-- screen 3 -->
                        <div class="phone__screen-b phone__screen--3">
                            <span class="number f--blue">
                                3
                            </span>
                            <h3 class="f--blue">
                                Facilitate 
                                <br>
                                all paperwork
                            </h3>
                            <p>
                                We will take care of all the paperwork for you and your business, under your supervision.
                            </p>
                        </div>
                        <!-- screen 4 -->
                        <div class="phone__screen-b phone__screen--4">
                            <span class="number f--blue">
                                4
                            </span>
                            <h3 class="f--blue">
                                Provide Route Planning 
                                and Book Loads
                            </h3>
                            <p>
                                Calendar View to see what loads you have for the day or drivers to see what loads they have assigned to them.
                            </p>
                        </div>
                        <!-- screen 5 -->
                        <div class="phone__screen-b phone__screen--5">
                            <span class="number f--blue">
                                5
                            </span>
                            <h3 class="f--blue">
                                Take Care of Communication With Brokers
                            </h3>
                            <p>
                                We will handle all the communication with brokers and shippers, keeping them up to date on all check ins needed, allowing you to provide top notch service to all your clients, without the hassle.
                            </p>
                        </div>
                        <!-- screen 6 -->
                        <div class="phone__screen-b phone__screen--6">
                            <span class="number f--blue">
                                6
                            </span>
                            <h3 class="f--blue">
                                OPTIMIZE RATE PERFORMANCE
                            </h3>
                            <p>
                                We are Spot Market Experts and 
                                Study market conditions daily, comparing your data with many other carriers, allowing us to make the best decisions in routing and load selection, in order to optimize your rates.
                            </p>
                        </div>
                    </div>
                    
                    <!-- icons -->
                    <button class="icon icon--1 active" data-number="1">
                        <svg 
                            version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             width="401.994px" height="401.994px" viewBox="0 0 401.994 401.994"
                             xml:space="preserve">
                            <g>
                                <path d="M394,154.175c-5.331-5.33-11.806-7.994-19.417-7.994H255.811V27.406c0-7.611-2.666-14.084-7.994-19.414
                                    C242.488,2.666,236.02,0,228.398,0h-54.812c-7.612,0-14.084,2.663-19.414,7.993c-5.33,5.33-7.994,11.803-7.994,19.414v118.775
                                    H27.407c-7.611,0-14.084,2.664-19.414,7.994S0,165.973,0,173.589v54.819c0,7.618,2.662,14.086,7.992,19.411
                                    c5.33,5.332,11.803,7.994,19.414,7.994h118.771V374.59c0,7.611,2.664,14.089,7.994,19.417c5.33,5.325,11.802,7.987,19.414,7.987
                                    h54.816c7.617,0,14.086-2.662,19.417-7.987c5.332-5.331,7.994-11.806,7.994-19.417V255.813h118.77
                                    c7.618,0,14.089-2.662,19.417-7.994c5.329-5.325,7.994-11.793,7.994-19.411v-54.819C401.991,165.973,399.332,159.502,394,154.175z"
                                    />
                            </g>
                        </svg>
                    </button>
                    <button class="icon icon--2" data-number="2">
                        <svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 297 297" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 297 297">
                              <g>
                                <path d="M148.51,117.216c32.317,0,58.608-26.291,58.608-58.608S180.827,0,148.51,0c-32.317,0-58.608,26.291-58.608,58.608   S116.193,117.216,148.51,117.216z"/>
                                <path d="m227.154,145.618c-0.025-0.008-0.073-0.026-0.098-0.032-7.631-1.864-30.999-5.133-30.999-5.133-2.638-0.812-5.457,0.585-6.406,3.188l-35.174,96.509c-2.029,5.567-9.903,5.567-11.932,0l-35.174-96.509c-0.766-2.102-2.75-3.42-4.876-3.42-0.504,0-24.531,3.369-32.53,5.358-21.858,5.435-35.645,26.929-35.645,49.329v80.302c0,12.034 9.756,21.79 21.79,21.79h184.782c12.034,0 21.79-9.756 21.79-21.79v-80.569c-0.001-22.303-14.328-42.096-35.528-49.023z"/>
                                <path d="m161.775,138.613c-1.404-1.53-3.456-2.299-5.532-2.299h-15.485c-2.076,0-4.129,0.77-5.532,2.299-2.173,2.368-2.489,5.789-0.946,8.462l8.278,12.479-3.875,32.69 7.631,20.3c0.744,2.042 3.631,2.042 4.375,0l7.631-20.3-3.875-32.69 8.278-12.479c1.541-2.673 1.225-6.094-0.948-8.462z"/>
                              </g>
                        </svg>
                    </button>
                    <button class="icon icon--3" data-number="3">
                        <svg 
                            version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             width="470.586px" height="470.586px" viewBox="0 0 470.586 470.586" style="enable-background:new 0 0 470.586 470.586;"
                             xml:space="preserve">
                            <g>
                                <path d="M327.081,0H90.234C74.331,0,61.381,12.959,61.381,28.859v412.863c0,15.924,12.95,28.863,28.853,28.863H380.35
                                    c15.917,0,28.855-12.939,28.855-28.863V89.234L327.081,0z M333.891,43.184l35.996,39.121h-35.996V43.184z M384.972,441.723
                                    c0,2.542-2.081,4.629-4.635,4.629H90.234c-2.55,0-4.619-2.087-4.619-4.629V28.859c0-2.548,2.069-4.613,4.619-4.613h219.411v70.181
                                    c0,6.682,5.443,12.099,12.129,12.099h63.198V441.723z M128.364,128.89H334.15c5.013,0,9.079,4.066,9.079,9.079
                                    c0,5.013-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079C119.285,132.957,123.352,128.89,128.364,128.89z
                                     M343.229,198.98c0,5.012-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15
                                    C339.163,189.901,343.229,193.968,343.229,198.98z M343.229,257.993c0,5.013-4.066,9.079-9.079,9.079H128.364
                                    c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15C339.163,248.914,343.229,252.98,343.229,257.993z
                                     M343.229,318.011c0,5.013-4.066,9.079-9.079,9.079H128.364c-5.012,0-9.079-4.066-9.079-9.079s4.067-9.079,9.079-9.079H334.15
                                    C339.163,308.932,343.229,312.998,343.229,318.011z"/>
                            </g>
                        </svg>
                    </button>
                    <button class="icon icon--4" data-number="4">
                        <svg 
                            version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" 			xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 511.999 511.999" style="enable-background:new 0 0 511.999 511.999;" xml:space="preserve">
                            <g>
                                <g>
                                    <path d="M112.867,423.071c-20.684,0-37.512-16.828-37.512-37.511c0-20.684,16.828-37.511,37.512-37.511h143.556
                                        c37.229,0,67.516-30.287,67.516-67.516s-30.288-67.515-67.516-67.515H155.182v30.004h101.241
                                        c20.684,0,37.512,16.828,37.512,37.512c0,20.684-16.828,37.511-37.512,37.511H112.867c-37.229,0-67.516,30.287-67.516,67.515
                                        s30.288,67.515,67.516,67.515h243.951v-30.004H112.867z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M138.953,76.519c-31.745-31.746-83.399-31.746-115.144,0c-31.746,31.746-31.746,83.398,0,115.144l57.572,57.573
                                        l57.573-57.573C170.699,159.918,170.699,108.265,138.953,76.519z M117.738,170.447l-36.356,36.355l-36.356-36.355
                                        c-20.047-20.047-20.047-52.665,0-72.712C55.049,87.713,68.216,82.7,81.382,82.7s26.332,5.012,36.356,15.035
                                        C137.785,117.783,137.785,150.4,117.738,170.447z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M80.989,97.915c-19.652,0-35.638,15.987-35.638,35.638c0,19.651,15.986,35.638,35.638,35.638s35.638-15.987,35.638-35.638
                                        C116.627,113.902,100.641,97.915,80.989,97.915z M80.989,139.187c-3.106,0-5.634-2.527-5.634-5.634
                                        c0-3.106,2.526-5.634,5.634-5.634s5.634,2.527,5.634,5.634C86.623,136.659,84.095,139.187,80.989,139.187z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M488.19,286.573c-31.745-31.746-83.399-31.746-115.144,0c-31.745,31.746-31.745,83.399,0,115.144l57.572,57.573
                                        l57.572-57.573C519.936,369.972,519.936,318.318,488.19,286.573z M466.975,380.502l-36.356,36.355l-36.356-36.355
                                        c-20.046-20.047-20.046-52.666,0-72.713c10.024-10.025,23.188-15.035,36.356-15.035c13.165,0,26.334,5.013,36.356,15.035
                                        C487.02,327.836,487.02,360.455,466.975,380.502z"/>
                                </g>
                            </g>
                            <g>
                                <g>
                                    <path d="M430.226,307.968c-19.651,0-35.638,15.987-35.638,35.638c0,19.651,15.986,35.638,35.638,35.638
                                        s35.638-15.987,35.638-35.638C465.864,323.955,449.878,307.968,430.226,307.968z M430.226,349.24
                                        c-3.106,0-5.634-2.527-5.634-5.634s2.526-5.634,5.634-5.634s5.634,2.527,5.634,5.634
                                        C435.859,346.713,433.333,349.24,430.226,349.24z"/>
                                </g>
                            </g>
                        </svg>
                    </button>
                    <button class="icon icon--5" data-number="5">
                        <svg 
                            version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                                <g>
                                    <g>
                                        <path d="M346,319c-5.522,0-10,4.477-10,10v69c0,27.57-22.43,50-50,50H178.032c-5.521,0-9.996,4.473-10,9.993l-0.014,19.882
                                            l-23.868-23.867c-1.545-3.547-5.081-6.008-9.171-6.008H70c-27.57,0-50-22.43-50-50V244c0-27.57,22.43-50,50-50h101
                                            c5.522,0,10-4.477,10-10s-4.478-10-10-10H70c-38.598,0-70,31.402-70,70v154c0,38.598,31.402,70,70,70h59.858l41.071,41.071
                                            c1.913,1.913,4.47,2.929,7.073,2.929c1.287,0,2.586-0.249,3.821-0.76c3.737-1.546,6.174-5.19,6.177-9.233L188.024,468H286
                                            c38.598,0,70-31.402,70-70v-69C356,323.477,351.522,319,346,319z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M366.655,0h-25.309C261.202,0,196,65.202,196,145.346s65.202,145.345,145.345,145.345h25.309
                                            c12.509,0,24.89-1.589,36.89-4.729l37.387,37.366c1.913,1.911,4.469,2.927,7.071,2.927c1.289,0,2.589-0.249,3.826-0.762
                                            c3.736-1.548,6.172-5.194,6.172-9.238v-57.856c15.829-12.819,28.978-29.012,38.206-47.102
                                            C506.687,190.751,512,168.562,512,145.346C512,65.202,446.798,0,366.655,0z M441.983,245.535
                                            c-2.507,1.889-3.983,4.847-3.983,7.988v38.6l-24.471-24.458c-1.904-1.902-4.458-2.927-7.07-2.927c-0.98,0-1.97,0.145-2.936,0.442
                                            c-11.903,3.658-24.307,5.512-36.868,5.512h-25.309c-69.117,0-125.346-56.23-125.346-125.346S272.23,20,341.346,20h25.309
                                            C435.771,20,492,76.23,492,145.346C492,185.077,473.77,221.595,441.983,245.535z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M399.033,109.421c-1.443-20.935-18.319-37.811-39.255-39.254c-11.868-0.815-23.194,3.188-31.863,11.281
                                            c-8.55,7.981-13.453,19.263-13.453,30.954c0,5.523,4.478,10,10,10c5.522,0,10-4.477,10-10c0-6.259,2.522-12.06,7.1-16.333
                                            c4.574-4.269,10.552-6.382,16.842-5.948c11.028,0.76,19.917,9.649,20.677,20.676c0.768,11.137-6.539,20.979-17.373,23.403
                                            c-8.778,1.964-14.908,9.592-14.908,18.549v24.025c0,5.523,4.478,10,10,10c5.523,0,10-4.477,9.999-10v-23.226
                                            C386.949,148.68,400.468,130.242,399.033,109.421z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M363.87,209.26c-1.86-1.86-4.44-2.93-7.07-2.93s-5.21,1.07-7.07,2.93c-1.86,1.86-2.93,4.44-2.93,7.07
                                            c0,2.64,1.071,5.22,2.93,7.08c1.86,1.86,4.44,2.92,7.07,2.92s5.21-1.06,7.07-2.92c1.86-1.87,2.93-4.44,2.93-7.08
                                            C366.8,213.7,365.729,211.12,363.87,209.26z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M275,310H64c-5.522,0-10,4.477-10,10s4.478,10,10,10h211c5.523,0,10-4.477,10-10S280.522,310,275,310z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M282.069,368.93C280.21,367.07,277.63,366,275,366s-5.21,1.07-7.07,2.93c-1.861,1.86-2.93,4.44-2.93,7.07
                                            s1.07,5.21,2.93,7.07c1.86,1.86,4.44,2.93,7.07,2.93s5.21-1.07,7.069-2.93c1.861-1.86,2.931-4.43,2.931-7.07
                                            C285,373.37,283.929,370.79,282.069,368.93z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M235.667,366H64c-5.522,0-10,4.477-10,10s4.478,10,10,10h171.667c5.523,0,10-4.477,10-10S241.189,366,235.667,366z"/>
                                    </g>
                                </g>
                                <g>
                                    <g>
                                        <path d="M210,254H64c-5.522,0-10,4.477-10,10s4.478,10,10,10h146c5.523,0,10-4.477,10-10S215.522,254,210,254z"/>
                                    </g>
                                </g>
                        </svg>
                    </button>
                    <button class="icon icon--6" data-number="6">
                        <svg 
                            version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                             width="932.179px" height="932.179px" viewBox="0 0 932.179 932.179"
                             xml:space="preserve">
                            <g>
                                <path d="M61.2,341.538c4.9,16.8,11.7,33,20.3,48.2l-24.5,30.9c-8,10.1-7.1,24.5,1.9,33.6l42.2,42.2c9.1,9.1,23.5,9.899,33.6,1.899
                                    l30.7-24.3c15.8,9.101,32.6,16.2,50.1,21.2l4.6,39.5c1.5,12.8,12.3,22.4,25.1,22.4h59.7c12.8,0,23.6-9.601,25.1-22.4l4.4-38.1
                                    c18.8-4.9,36.8-12.2,53.7-21.7l29.7,23.5c10.1,8,24.5,7.1,33.6-1.9l42.2-42.2c9.1-9.1,9.9-23.5,1.9-33.6l-23.1-29.3
                                    c9.6-16.601,17.1-34.3,22.1-52.8l35.6-4.1c12.801-1.5,22.4-12.3,22.4-25.1v-59.7c0-12.8-9.6-23.6-22.4-25.1l-35.1-4.1
                                    c-4.801-18.3-12-35.8-21.199-52.2l21.6-27.3c8-10.1,7.1-24.5-1.9-33.6l-42.1-42.1c-9.1-9.1-23.5-9.9-33.6-1.9l-26.5,21
                                    c-17.2-10.1-35.601-17.8-54.9-23l-4-34.3c-1.5-12.8-12.3-22.4-25.1-22.4h-59.7c-12.8,0-23.6,9.6-25.1,22.4l-4,34.3
                                    c-19.8,5.3-38.7,13.3-56.3,23.8l-27.5-21.8c-10.1-8-24.5-7.1-33.6,1.9l-42.2,42.2c-9.1,9.1-9.9,23.5-1.9,33.6l23,29.1
                                    c-9.2,16.6-16.2,34.3-20.8,52.7l-36.8,4.2c-12.8,1.5-22.4,12.3-22.4,25.1v59.7c0,12.8,9.6,23.6,22.4,25.1L61.2,341.538z
                                     M277.5,180.038c54.4,0,98.7,44.3,98.7,98.7s-44.3,98.7-98.7,98.7c-54.399,0-98.7-44.3-98.7-98.7S223.1,180.038,277.5,180.038z"/>
                                <path d="M867.699,356.238l-31.5-26.6c-9.699-8.2-24-7.8-33.199,0.9l-17.4,16.3c-14.699-7.1-30.299-12.1-46.4-15l-4.898-24
                                    c-2.5-12.4-14-21-26.602-20l-41.1,3.5c-12.6,1.1-22.5,11.4-22.9,24.1l-0.799,24.4c-15.801,5.7-30.701,13.5-44.301,23.3
                                    l-20.799-13.8c-10.602-7-24.701-5-32.9,4.7l-26.6,31.7c-8.201,9.7-7.801,24,0.898,33.2l18.201,19.399
                                    c-6.301,14.2-10.801,29.101-13.4,44.4l-26,5.3c-12.4,2.5-21,14-20,26.601l3.5,41.1c1.1,12.6,11.4,22.5,24.1,22.9l28.1,0.899
                                    c5.102,13.4,11.801,26.101,19.9,38l-15.699,23.7c-7,10.6-5,24.7,4.699,32.9l31.5,26.6c9.701,8.2,24,7.8,33.201-0.9l20.6-19.3
                                    c13.5,6.3,27.699,11,42.299,13.8l5.701,28.2c2.5,12.4,14,21,26.6,20l41.1-3.5c12.6-1.1,22.5-11.399,22.9-24.1l0.9-27.601
                                    c15-5.3,29.199-12.5,42.299-21.399l22.701,15c10.6,7,24.699,5,32.9-4.7l26.6-31.5c8.199-9.7,7.799-24-0.9-33.2l-18.301-19.399
                                    c6.701-14.2,11.602-29.2,14.4-44.601l25-5.1c12.4-2.5,21-14,20-26.601l-3.5-41.1c-1.1-12.6-11.4-22.5-24.1-22.9l-25.1-0.8
                                    c-5.201-14.6-12.201-28.399-20.9-41.2l13.699-20.6C879.4,378.638,877.4,364.438,867.699,356.238z M712.801,593.837
                                    c-44.4,3.801-83.602-29.3-87.301-73.699c-3.801-44.4,29.301-83.601,73.699-87.301c44.4-3.8,83.602,29.301,87.301,73.7
                                    C790.301,550.938,757.199,590.138,712.801,593.837z"/>
                                <path d="M205,704.438c-12.6,1.3-22.3,11.899-22.4,24.6l-0.3,25.3c-0.2,12.7,9.2,23.5,21.8,25.101l18.6,2.399
                                    c3.1,11.301,7.5,22.101,13.2,32.301l-12,14.8c-8,9.899-7.4,24.1,1.5,33.2l17.7,18.1c8.9,9.1,23.1,10.1,33.2,2.3l14.899-11.5
                                    c10.5,6.2,21.601,11.101,33.2,14.5l2,19.2c1.3,12.6,11.9,22.3,24.6,22.4l25.301,0.3c12.699,0.2,23.5-9.2,25.1-21.8l2.3-18.2
                                    c12.601-3.101,24.601-7.8,36-14l14,11.3c9.9,8,24.101,7.4,33.201-1.5l18.1-17.7c9.1-8.899,10.1-23.1,2.301-33.2L496.6,818.438
                                    c6.6-11,11.701-22.7,15.201-35l16.6-1.7c12.6-1.3,22.299-11.9,22.4-24.6l0.299-25.301c0.201-12.699-9.199-23.5-21.799-25.1
                                    l-16.201-2.1c-3.1-12.2-7.699-24-13.699-35l10.1-12.4c8-9.9,7.4-24.1-1.5-33.2l-17.699-18.1c-8.9-9.101-23.102-10.101-33.201-2.3
                                    l-12.101,9.3c-11.399-6.9-23.6-12.2-36.399-15.8l-1.601-15.7c-1.3-12.601-11.899-22.3-24.6-22.4l-25.3-0.3
                                    c-12.7-0.2-23.5,9.2-25.101,21.8l-2,15.601c-13.199,3.399-25.899,8.6-37.699,15.399l-12.5-10.2c-9.9-8-24.101-7.399-33.201,1.5
                                    l-18.2,17.801c-9.1,8.899-10.1,23.1-2.3,33.199l10.7,13.801c-6.2,11-11.1,22.699-14.3,35L205,704.438z M368.3,675.837
                                    c36.3,0.4,65.399,30.301,65,66.601c-0.4,36.3-30.301,65.399-66.601,65c-36.3-0.4-65.399-30.3-65-66.601
                                    C302.1,704.538,332,675.438,368.3,675.837z"/>
                            </g>
                        </svg>
                    </button>
                </div>

                <!-- Navigation -->
                <div class="features__nav">
                    <button class="nav-button active" data-number="1">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                    <button class="nav-button" data-number="2">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                    <button class="nav-button" data-number="3">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                    <button class="nav-button" data-number="4">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                    <button class="nav-button" data-number="5">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                    <button class="nav-button" data-number="6">
                        <svg width="100%" height="100%" viewBox="0 0 44 42" version="1.1" 
                            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:1.41421;"><path d="M34.69,0l8.389,25.269l-21.44,15.786l-21.639,-15.512l8.066,-25.374l26.624,-0.169Z"/>
                        </svg>
                    </button>
                </div>

                <!-- INFOBAR -->
                <div class="infoBar">	
                    <img src="{{asset('assets/images/information.svg')}}" alt="infomation">
                    Click on icons to change feature
                </div>			
            </div>
        </div>
    </div>
</section>
<!-- /Features -->

    <!-- GET IT FROM  -->
    <section class="google">
        <div class="container">
            <div class="row">
                <div class="col-md-12 col-sm-12 getin">
                    <h2 class="big f--white">
                            Download The App Today! 
                        <!--span class="small">
                            and make your job easier.
                        </span -->
                    </h2>
                    <a href="https://play.google.com/store/apps/details?id=com.cleaningapp" class="get">
                        <img src="{{asset('assets/images/en_badge_web_generic.png')}}" alt="">
                    </a>
                    <a href="https://itunes.apple.com/us/app/merge-transit-spot-market-app/id1434054240?mt=8" class="get">
                        <img src="{{asset('assets/images/AppStoreIcon.png')}}" alt="">
                    </a>
                </div>
            </div>
        </div>
    </section>
    <!-- /GET IT FROM -->
        
    
@endsection