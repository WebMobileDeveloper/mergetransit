<?php
	...
	if ( $socials_slugs ) {
		...
		echo '<div class="module-11 module' . ( $style ? ' has-background' : '' ) . '"' . $hash . $style . '>';
							...
							foreach ( $socials as $social ) {
								...
								if ( $social[ 'url' ] ) {
									...
									if ( $social[ 'label' ] || $social[ 'icon-post-id' ] ) {
										...
										$page_url = ( isset( $_SERVER['HTTPS'] ) ? 'https' : 'http') . '://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
								
								
										//Display the social open tag.
										echo '<a' .

											' href="' . esc_url( $social[ 'url' ] . $page_url ) . '"' .

											' class="social flex middle nowrap"' .
