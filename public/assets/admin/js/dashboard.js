$(document).ready(function () {
    ...
    var select_staticObj = $(".statistic #period_search").get(0);

    select_staticObj.addEventListener("change", function (event) {
        event.preventDefault();
        var type = this.value;
        get_detail_info(type);

    });


    if (user_role != 3 && user_role != 4) {
        var select_period = $(".period_total #period_search").get(0);
        select_period.addEventListener("change", function (event) {
            event.preventDefault();
            var type = this.value;
            get_detail_bybroker(type);
        });
    }
});
