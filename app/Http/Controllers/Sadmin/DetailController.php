...
class DetailController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('sadmin.details.index', compact('details','s'));
    }
    ...
    public function create()
    {
        ...
        return view('sadmin.details.detailAdd', compact('drivers','contacts'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($success) {
            return redirect('sadmin/details');
        } else {
            return redirect('sadmin/details');
        }
    ...
    public function edit($id)
    {
        ...
        return view('sadmin.details.detailEdit', compact('drivers','detail','contacts','detail_origin','detail_dest'));
    }
    ...
    /*private function calcLength(){
        ...
        for ($i = 0; $i < count($this->coordArr)-1  ; $i++) { 
            ...
            if ( !$origin || !$dest )
            {
                ...
                $this->GetDrivingDistance($origin['lat'], $dest['lat'], $origin['long'], $dest['long']);
            }           
        }       
    } 
    private function get_coordinates($index)
    {
        $this->zipCount++;
        $zipcode=trim($this->zip_array[$index]);
        // $address = urlencode($city.','.$street.','.$province);
        $url = "https://maps.google.com/maps/api/geocode/json?address=$zipcode&sensor=false&region=US&key=".$this->apiKey;
        $ch = curl_init();
        ...
        else
        {
            ...
            $this->coordArr[$index] = $return;
            if($this->zipCount==count($this->zip_array)){
                $this->calcLength();
            }
        }
    }
